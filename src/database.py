...
from sqlalchemy import DateTime, Integer, String, UniqueConstraint, create_engine, func
...
class SyncSpimexTrade(Base):
    __tablename__ = "sync_spimex_trades"
    __table_args__ = (
        UniqueConstraint(
            "instrument_code",
            "instrument_name",
            "source_file",
            name="uq_sync_spimex_trade_identity",
        ),
    )
...
class SyncDatabase:
    def __init__(self, settings: Settings) -> None:
        self.engine = create_engine(settings.postgres_dsn_psycopg, pool_pre_ping=True)
    ...
    def upsert_trade_rows(self, rows: Sequence[TradeRow], batch_size: int = 1000) -> int:
        unique_rows = self._deduplicate_rows(rows)
        if not unique_rows:
            logger.warning("Нет строк для загрузки в БД")
            return 0
        ...
        with self.session_factory() as session:
            for start in range(0, len(unique_rows), batch_size):
                ...
                query = insert(SyncSpimexTrade).values(payload)
                query = query.on_conflict_do_update(
                    constraint="uq_sync_spimex_trade_identity",
                    set_={
                        "bulletin_url": query.excluded.bulletin_url,
                        "trade_date": query.excluded.trade_date,
                        "updated_at": func.now(),
