...
def main(pages: int | None, limit: int | None, batch_size: int) -> None:
    ...
    inserted = database.upsert_trade_rows(rows=enriched_rows, batch_size=batch_size)
    database.close()
    ...
    logging.info("Итог sync pipeline: файлов=%s, строк=%s, upsert=%s", len(files), len(enriched_rows), inserted)
    logging.info("Общее время sync pipeline: %.2f сек", total_elapsed)
...
if __name__ == "__main__":
    ...
    args = cli.parse_args()
    main(pages=args.pages, limit=args.limit, batch_size=args.batch_size)
