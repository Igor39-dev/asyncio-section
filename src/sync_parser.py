...
class SyncBulletinParser:
    code_pattern = re.compile(r"\b[A-ZА-Я0-9][A-ZА-Я0-9\-./]{2,}\b")

    def parse_files(self, files: list[Path]) -> list[ParsedTradeRecord]:
    ...
    def extract_trade_rows(self, files: list[Path]) -> list[TradeRow]:
        rows: list[TradeRow] = []
        for file_path in files:
            try:
    ...
    def _parse_file(self, file_path: Path) -> ParsedTradeRecord:
        file_type = self._detect_file_type(file_path)
        if file_type == "pdf":
